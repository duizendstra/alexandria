...
- **docs**, **blueprints**, **ci**: Added the workshop playbook for building an AI-assisted Google Calendar add-on, together with the two blueprints it teaches from. [`docs/07-playbooks/workshop-calendar-time-tracker.md`](docs/07-playbooks/workshop-calendar-time-tracker.md) walks an audience through the same feature twice: Level 1 is an Apps Script prototype (`blueprints/google-addon/appsscript/`) that needs no local toolchain and no billing account, and Level 2 rebuilds it as a Go Alternate Runtime on Cloud Run (`blueprints/google-addon/go/`) containerised with `ko`, so the second half demonstrates `go/platform/web` and `go/slog-gcp` inside a service the participant has already reasoned about. The Go blueprint passed its Gemini API key in the request URL, and since a transport failure returns a `*url.Error` whose message embeds the whole URL — and that error was both logged and written into the HTTP response body — a single failed call published the key twice; the key now travels in the `X-Goog-Api-Key` header, pinned by a regression test with a control arm. The service also called `http.ListenAndServe`, which sets no timeouts at all, and now builds an `http.Server` with named read-header, read, write and idle timeouts and recovers panics through `web.RecoveryMiddleware`. None of those faults could have been reported, because every Go job in CI enumerates modules with `find go/` and blueprints live outside it; that boundary is deliberate and is now stated in `blueprints/README.md` and at the discovery step itself, rather than left to be inferred from a `find` root.
- **docs**, **ci**: The `go/README.md` module index is complete again and no longer carries a version column of its own. It listed 16 of the 32 published modules — all ten `go/iac/*` modules, `go/governance` and five `go/platform/*` modules were absent — and its second column was headed **Status** but held a version, `v0.0.1` for every row, a number no module has carried since the repository's first week. Nothing in the repository referenced the file and no gate covered it, so it drifted silently while the root `README.md` was held in step with `.release-please-manifest.json` by two CI gates. Versions and import paths now live in exactly one gated index: `go/README.md` lists all 32 modules with their descriptions and points at the root index for the rest. A new `Module hygiene` step fails CI when a module under `go/` is missing from the table, mirroring the gate that already protects the root index, so this one cannot go half-stale again (#313).
- **release plumbing**: Releases are re-anchored against the current commit history. `go/google` and `go/governance` had every recent tag deleted when history was rewritten, which left release-please with no boundary tag: it re-walked history from far too early and proposed a changelog of 26 entries for `go/governance` where only one commit had touched the module since its last release, together with a `2.0.0` major bump swept up from a breaking-change footer that had already shipped in `v1.0.0`. The open release PR is therefore discarded rather than merged. `go/google v0.4.3`, `go/governance v1.0.2` and `go/iac/observability v0.3.2` are cut by hand so that a tag matching the manifest exists again; release-please computes correctly from there. The versions whose tags no longer exist cannot be reproduced from source and are withdrawn with `retract` directives — `[v0.2.0, v0.4.2]` for `go/google`, `[v0.4.0, v1.0.1]` for `go/governance` — so `go get -u` will not select them. No version number is reused: the module proxy is keyed by version string and permanently serves the zip it cached first, so re-tagging a burned number would hand consumers the old code under the new name. No exported API changed in either module.
- **go/google/workspace/sheets**, **go/iac/observability**, **.agents**: The runnable `ReplaceTab` example no longer names a downstream consumer's process in its spreadsheet-ID placeholder. The example was written from a real consumer's sync, and the anonymization pass that replaced its addresses, domains and folder ID left one domain noun fused into a placeholder-shaped string, where it read as generic to anyone who did not know the domain; it is now `existing-spreadsheet-id`. Separately, the Public Repo Hygiene rule in `.agents/AGENTS.md` no longer names a private organization as its own example — which is precisely what the rule forbids. The uptime-check configuration example in `go/iac/observability` likewise used a real organization's stack reference where the field's own documented format (`org/project/stack`) was all the example needed. Example- and docs-only; no library behaviour changed.
- **docs**: Every Go module README now carries a **Consumers & Load-Bearing Promises** section — the consumer archetypes a module serves and the behaviours consumers pin, each one derived from a test that already holds it. Breaking a listed promise is a major version bump. Consumers are named as archetypes rather than as repositories, because Alexandria is public. Three modules had no README at all — `go/iac/pulumi/runner`, `go/platform/cache` and `go/platform/web` — and now have one, so the section covers every published module. The reviewer's fixed checklist is now written down in `docs/07-playbooks/pr-review-checklist.md` (#230).
- **docs**, **skills**: The protocol between Alexandria and a downstream consumer repository is now written down once, in `docs/07-playbooks/ascension-and-consumption.md`, and taught operationally by a new `skills/alexandria-consumer` skill. It covers both directions: consuming a module by tag behind blocking consumer contract tests that gate the bump, and ascending a staged package — generic, module-shaped, with exactly one real consumer — through a scaffold Alexandria writes and the consumer team implements, merged and tagged in the same session so the staged copy can be deleted rather than left as a second live copy. Cross-repository suggestions travel as issues labelled `advisory` or `ascension`; the labels are generic and the repository is named in the issue body, because label names are public and would otherwise publish the relationship. The PR review gate is named here; its checklist is tracked separately (#230).
- **go/google/workspace/drive**, **go/governance/gate**: Test fixtures no longer carry identifiers from a downstream deployment. A real organization's mail domain appeared in five fixture addresses across two modules — and in three exported-looking test constant names — and a real shared-drive name appeared in a third. All are now RFC 2606 `example.com` addresses and a generic drive name; the constants are named for the role they play in the test rather than for whoever the fixture was copied from. Test-only; no library behaviour changed.
- **docs**, **scripts/okf-lint.py**: The vault now conforms to OKF v0.2 §3.1/§8 on reserved filenames. `index.md` and `log.md` are not concept documents, so the eight domain indexes no longer carry the Alexandria frontmatter schema — the local rule that required it inverted the spec rule it claimed to extend. A new bundle-root `docs/index.md` declares `okf_version: "0.2"`, the only place §12 permits frontmatter in an index, and lists the eight domains; every index lists its concepts as §8 bullets — a linked title followed by the concept's own summary. `okf-lint` gained the other half of the rule: it rejects frontmatter on a non-root index, a root index that declares anything besides `okf_version`, an index with no headings or no entries, and a `log.md` section heading that is not an ISO 8601 date; the `index` document type is gone. Alexandria's required fields remain an extension over §4.1, not a deviation. `check-links.sh` now strips fenced code blocks before extracting links, so example markdown in a spec document no longer reports its placeholders as broken (#265).
- **go/iac/pulumi/gcpinfra**: Data-bearing resources are now created protected at both layers by default. New package `lifecycle` carries the one decision every block needs: blocks set `pulumi.Protect` (the engine refuses the delete at *preview*) alongside the provider's own `deletionPolicy`/`deletionProtection` (API-enforced, but only fails mid-apply) on folders, tag keys, projects, BigQuery datasets and native tables, Firestore databases and seeded documents, Artifact Registry repositories and Secret Manager secrets. `tables.Apply`/`ApplyExternal` previously sent `deletionProtection: false` explicitly, overriding the provider's safe default, so a table rename — a delete plus a create, not an update — dropped its rows without warning. Every `Apply` now takes optional `...lifecycle.Option`; a test or preview stack passes `lifecycle.Ephemeral()` to clear both layers so `pulumi destroy` works without `pulumi state unprotect` first. External tables, secret versions, service accounts and IAM bindings stay unprotected deliberately — their source of truth is outside the stack. Existing call sites compile unchanged, and no logical name changed, but the first `up` after upgrading writes protection into state; **deploy it on its own before renaming anything**, since `Protect` cannot guard the very update that introduces it. `firestore.DatabaseConfig.DeleteProtection` is now validated against the two states the API accepts (#285).
- **go/iac/pulumi/runner**: The test suite's fake `pulumi` stubs are written once from `TestMain`, before any parallel test starts, instead of inside each `t.Parallel()` test. On Linux a stub was exec'd while a child forked by a sibling test still held the write descriptor it had inherited, and the kernel refused the exec with `ETXTBSY` (`text file busy`) — the flake that failed `TestRunner_SecretsNeverNamed` on CI. Test-only; no library behaviour changed (#291).
...
- **go/governance/gate**: An unknown or empty `Policy` — including a zero-valued `Gate` — now evaluates to `VerdictBlocked` with a failed `gate-policy` result explaining why, instead of failing open as `VerdictPass`; added `ErrUnknownPolicy`, `Policy.Validate` and `Gate.Validate` to reject a misconfigured policy at construction time (#246).
- **go/slog-gcp**: The Cloud Logging reserved fields (`logging.googleapis.com/trace`, `spanId`, `trace_sampled`, `insertId`) are now emitted at the payload root even when the logger has an active `WithGroup`; previously they were nested under the group object and Cloud Logging ignored them, silently losing trace correlation and insertId deduplication. Output without a group is unchanged (#256).
- **go/iac/observability**: Uptime targets that reference the same stack now share one `StackReference` — the per-target read registered a duplicate URN and aborted the whole update (#249).
- **go/iac/observability**: All stack reads in a run — the governance stack and every uptime target — now share one `StackReference` per stack name; the per-consumer reads registered a duplicate URN and aborted the whole update whenever two of them named the same stack (#249).
- **go/retry/gcp**: `Classify` no longer treats every `net.Error` as transient. `*url.Error` satisfies `net.Error` for every HTTP client failure, so permanent causes — TLS handshake and certificate errors, cancelled contexts, and any cause that is neither a timeout nor a dropped, refused or unreachable connection — were retried through the full backoff schedule; they now fail fast, while timeouts, connection reset/refused/aborted, unreachable routes, temporary resolver failures and dropped keep-alive connections stay retryable (#254).
- **go/discovery/privacyfilter**: Redaction now matches patterns case-insensitively (`PASSWORD=` redacts like `password=`) and runs over the title and every metadata value as well as the body, so a secret no longer reaches the index by changing case or by living outside the content field. Redaction marker text and whole-line replacement are unchanged (#245).
- **go/platform/coordination**: `filelock`'s contention test sizes the last caller's reclaim age to the window it must cover — five fsyncs plus that caller's whole patience, not one fsync — and skips with the measured stamp-to-look gap when the host stalls past that budget, instead of failing on a premise it never checked (#269).
- **go/google/workspace/sheets**: `SyncDocument` with `PruneTabs` no longer deletes a tab it just synced by GID — a tab is kept when its title matches a spec or its sheet ID matches a synced tab — and a failed prune now returns an error instead of being swallowed (#243).
- **go/google/workspace/sheets**: A column that mixes formula and text cells is now written per run of rows, so only its formula cells are sent `USER_ENTERED`; text cells that start with `=`, `+`, `-` or `@` stay literal under `RAW` instead of being evaluated. Pure text and pure formula columns are written exactly as before (#244).
- **go/google/workspace/drive**: Added `EscapeQueryValue`, which doubles backslashes before escaping single quotes; `FindSharedDriveByName`, `FindFolder` and `FindFolderByProperty` now build their Drive `q` string through it, so a value with a trailing backslash or a backslash-quote is matched literally instead of breaking or rewriting the query (#255).
- **go/iac/delivery**: The registry writer grant now lands on the identity builds actually run as. New optional config key `buildServiceAccount` (an email) makes that account the sole `artifactregistry.writer` grantee; when unset the blueprint grants both the Compute default SA (`<number>-compute@developer.gserviceaccount.com`, the build identity of projects on Cloud Build's 2024 default) and the legacy Cloud Build SA, so the first image push is no longer denied on new-default projects. The legacy grant keeps its logical name; the Compute grant is a new resource (`build-ar-writer-compute`) (#253).
- **go/iac/pulumi/gcpinfra**: Config values that become Pulumi logical names are now checked for repeats before any resource is created — `tables.Apply`/`ApplyExternal` (table `Name`), `firestore.ApplyDocuments` (`DocumentID`, across collections), `budgets.Config.Validate` (alert email) and `folders.Apply` (a child named like the root) return the new sentinels `ErrDuplicateName`/`ErrDuplicateExternalName`, `ErrDuplicateDocumentID`, `ErrDuplicateAlertEmail` and `ErrDuplicateFolder` naming the offending value, instead of letting the engine abort the whole `pulumi up` with a duplicate-URN error the SDK mocks never raise. Logical names are unchanged, so existing stacks see no replacements (#248).
- **go/iac/identity**, **go/iac/governance**, **go/iac/delivery**: A present-but-malformed optional config block (`secrets`, `serviceAccounts`, `impersonators`, `tagKeys`, `repositories`, `consumerWorkloadStacks`) now aborts `Apply` with an error naming the key instead of being read as empty — which let the next update delete every resource the block had declared. An absent key still applies cleanly (#257).
...
- **CI Tooling**: The `pre-push` hook now scopes its quality gate to the commits being pushed instead of sweeping every module on every push — a documentation-only push runs `okf-lint` and the link check alone, a change inside one module runs that module, and any change outside `go/` that is not markdown — CI config, hooks, `scripts`, `contracts`, `blueprints`, `skills`, root dotfiles — falls back to the full sweep. Nested modules resolve to the nearest enclosing `go.mod`, so `go/retry/gcp` no longer drags in `go/retry`. The broken-internal-link check moved out of `ci.yml` into `scripts/check-links.sh`, so CI and the hook run one implementation; it is POSIX sh rather than GNU `grep -oP` plus bash process substitution, which never ran on the team's macOS shells. `just check` stays the full local gate and CI stays authoritative — the ruleset on `main` requires its checks regardless of what the hook chose to run (#233).
...
- **CI Tooling**: The per-module `go` job now fails on files that are not `gofmt`-clean (`gofmt -l .` must print nothing) and runs `GOOS=windows GOARCH=amd64 go vet ./...` so every module keeps compiling on native Windows, not just Linux and macOS.
