...
- **docs**, **blueprints**: Added the workshop playbook for building an AI-assisted Google Calendar add-on, together with the two blueprints it teaches from. [`docs/07-playbooks/workshop-calendar-time-tracker.md`](docs/07-playbooks/workshop-calendar-time-tracker.md) walks a workshop audience through the same feature twice: Level 1 is an Apps Script prototype (`blueprints/google-addon/appsscript/`) that needs no local toolchain and no billing account, and Level 2 rebuilds it as a Go Alternate Runtime on Cloud Run (`blueprints/google-addon/go/`) containerised with `ko`, so the second half demonstrates `go/platform/web` and `go/slog-gcp` inside a service the participant has already reasoned about. The blueprints are teaching material, not published modules: `blueprints/google-addon/go` is a standalone module outside `go/`, so it is absent from the module index, the release manifest and the coverage ratchet — and, for the same reason, from every CI job, all of which enumerate modules under `go/` only.
