# gcp (`go/platform/retry/gcp`)

`gcp` is an extension package for `go/platform/retry` providing automated error classification and smart retry behaviors specifically tailored for Google Cloud Platform (GCP) and Google Workspace API client libraries.
...
```bash
go get github.com/duizendstra/alexandria/go/platform/retry/gcp
```
