...
`go/platform/retry` provides robust, thread-safe backoff and retry policies for cloud-native Go microservices.
...
- **Keep-Alive Socket Draining**: Pre-consumes up to 512KB of failed response bodies before closing them to preserve and reuse TCP keep-alive sockets.
...
```bash
go get github.com/duizendstra/alexandria/go/platform/retry
```
...
1. **Keep-Alive Protection**: Calling `Close()` on a response body before its bytes are fully consumed tears down the underlying TCP connection. The `retry.Transport` automatically copies and discards trailing bytes (up to 512KB) prior to closure to permit connection pooling.
