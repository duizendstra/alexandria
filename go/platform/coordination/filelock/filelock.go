...
// Locker gives mutual exclusion per subject, using an atomic filesystem lock file
// created with O_CREATE|O_EXCL. The lock is released when the returned function is
// called, and also when the process receives an interruption signal.
type Locker struct {
	...
	// Signals are the signals that trigger cleanup. Nil means SIGINT and SIGTERM.
	Signals []os.Signal
...
// Acquire takes the lock for a subject. The returned release function is safe to call
// more than once. It returns coordination.ErrLocked when the lock is already held.
func (l *Locker) Acquire(subject string) (func(), error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrExist) {
			return nil, fmt.Errorf("%s (%s): %w", subject, path, coordination.ErrLocked)
		}
...
func pathFor(dir, prefix, subject string) (string, error) {
	if err := checkSubject(subject); err != nil {
		return "", err
	}

	return filepath.Join(dir, prefix+subject), nil
}
